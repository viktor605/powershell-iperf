...
if ($posh -match "SpeedTest for Windows 7-10.ps1") {Stop-Process -Id $pid} elseif ($posh -match "SpeedTest for Windows XP auto.ps1") {Write-Host; Write-Host "Будте внимательны! В данный момент скрипт также работает в автоматическом скрытом режиме."; Read-Host -Prompt " Нажмите Enter для продолжения"}
